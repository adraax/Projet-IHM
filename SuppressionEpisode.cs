...
{
    public partial class SuppressionEpisode : Form
    {
        private string commande;
        private ConnexionOracle bdd;
        private DataSet ds;

        public SuppressionEpisode()
        {
            InitializeComponent();
            bdd = new ConnexionOracle();
        }

        private void SuppressionEpisode_Load(object sender, EventArgs e)
        {
        ...
        {
            commande = string.Format("DELETE FROM PROJET_IHM_EPISODE WHERE NOMSERIE='{0}' AND NUMEROSAISON={1} AND NUMEPISODE={2}", listeSerie.SelectedItem.ToString().Replace("'", "''"), Decimal.Parse(listeSaison.SelectedItem.ToString().Substring(listeSaison.SelectedItem.ToString().Length - 1)), Decimal.Parse(listeEpisode.SelectedItem.ToString().Substring("Episode ".Length, 1)));
            bdd.Delete(commande);
